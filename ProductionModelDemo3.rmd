...
To use a production model for stock assessment, we use statistical methods to estimate the parameters r, K, and q (the constant of proportionality between q CPUE and biomass) that provide the best fit of the model to the CPUE data[^1]. To see how this works, we will fit the data for North Atlantic swordfish from ICCAT(1999).
...
**5. Fit the model to the swordfish data ** First use the sliders to explore what values of r, K and q best fit the data. After you have tried several values, select "Solve" to let R solve for the values that minimize the sum of squares (SS). Does the fit look reasonable?
...
**6. Projections** In the following, you can select a harvest control rule and project the population forward 10 years with our estimated values of r and K, with either a fixed catch quota or a catch fraction. 
